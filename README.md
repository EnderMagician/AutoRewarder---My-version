...
Download `AutoRewarder-Setup.exe` from the [latest release](https://github.com/EnderMagician/AutoRewarder---My-version/releases/latest) and run it. The installer will verify all dependencies and install the app for you.

**Portable Way:**
Download `AutoRewarder.zip` from the [latest release](https://github.com/EnderMagician/AutoRewarder---My-version/releases/latest) and extract it to any folder (e.g., a USB drive). Run the executable. All your settings and profiles will be saved locally inside the `config` folder.
